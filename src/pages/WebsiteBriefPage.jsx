...
export default function WebsiteBriefPage({ embedded = false }) {
  useDocumentTitle(embedded ? 'Website Brief | Dashboard' : 'Website Brief | San Jose Logo Design');
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      navigate(embedded ? '/dashboard/website-projects' : '/thankyou');
    } catch (error) {
  ...
  return (
    <>
      ...
      <section
        className={`contact-section ${embedded ? 'py-3' : 'py-5'} ${styles.page} ${embedded ? styles.embedded : ''}`}
      >
        <div className={embedded ? 'container-fluid px-2 px-md-3' : 'container'}>
          {!embedded ? (
