  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (paymentIntent.status === 'succeeded') {
        const confirmData = await apiPost(`/payment-requests/${paymentRequestId}/stripe/confirm`, {
          payment_intent_id: paymentIntent.id,
        });
        onSuccess(confirmData?.login_token || null);
      }
    } catch (err) {
      onError(err.message);
      setPaying(false);
        ...
        cashAppBtn.on('confirm', async () => {
          try {
            ...
            if (error) {
              ...
              const cashConfirm = await apiPost(
                `/payment-requests/${paymentRequestId}/cashapp/confirm`,
                { payment_intent_id: paymentIntent.id }
              );
              onSuccess(cashConfirm?.login_token || null);
            }
          } catch (err) {
            onError(err.message);
          }
  ...
  const handleFallbackPay = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        const cashFallback = await apiPost(
          `/payment-requests/${paymentRequestId}/cashapp/confirm`,
          { payment_intent_id: paymentIntent.id }
        );
        onSuccess(cashFallback?.login_token || null);
      }
    } catch (err) {
      onError(err.message);
    } finally {
