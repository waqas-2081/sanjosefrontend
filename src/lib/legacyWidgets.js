...
let swiperInstances = [];
let portfolioSwiperInstances = [];
const teardownFns = [];

const PORTFOLIO_MOBILE_MQ = '(max-width: 991.98px)';

function registerTeardown(fn) {
  teardownFns.push(fn);
}

function destroyPortfolioTabSwipers() {
  portfolioSwiperInstances.forEach((s) => {
    try {
      s.destroy(true, true);
    } catch {
      /* ignore */
  ...
  portfolioSwiperInstances = [];
}

function destroySwipers() {
  destroyPortfolioTabSwipers();
...
function initPortfolioTabSwipers() {
  if (typeof window.Swiper === 'undefined') return;

  if (!window.matchMedia(PORTFOLIO_MOBILE_MQ).matches) {
    destroyPortfolioTabSwipers();
    return;
  }

  document.querySelectorAll('.portfolio-tab-swiper').forEach((el) => {
    ...
    portfolioSwiperInstances.push(instance);
  });
}

let portfolioResizeListenerAttached = false;

function ensurePortfolioSwiperResizeListener() {
  if (portfolioResizeListenerAttached || typeof window.matchMedia !== 'function') return;
  portfolioResizeListenerAttached = true;
  const mq = window.matchMedia(PORTFOLIO_MOBILE_MQ);
  const onChange = () => refreshPortfolioTabSwipers();
  mq.addEventListener('change', onChange);
  registerTeardown(() => {
    mq.removeEventListener('change', onChange);
    portfolioResizeListenerAttached = false;
  });
}
...
function initSwipers() {
  ...
  document.querySelectorAll('.featured-stories-swiper').forEach((el) => {
    const reduceMotion =
      typeof window.matchMedia === 'function' &&
      window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const instance = new window.Swiper(el, {
...
function initCompareSlider() {
  ...
  if (!container || container.dataset.compareSlider === 'react') return;
...
export function refreshPortfolioTabSwipers() {
  destroyPortfolioTabSwipers();
  initPortfolioTabSwipers();
}

export function refreshSwipers() {
  destroySwipers();
  initSwipers();
  initPortfolioTabSwipers();
}
...
export function mountLegacyWidgets() {
  requestAnimationFrame(() => {
    ...
    initSwipers();
    initPortfolioTabSwipers();
    ensurePortfolioSwiperResizeListener();
    initMarquee();
    initPortfolioTabs();
    initCompareSlider();
    initOwl();
