...
function isPackageCheckoutTrigger(node) {
  if (!node) return false;
  if (node.dataset?.packageCheckout === 'true') return true;
  return Boolean(node.closest?.('[data-package-checkout="true"]'));
}

function isQuoteTrigger(node) {
  if (!node) return false;
  if (isPackageCheckoutTrigger(node)) return false;
